...
def get_hfi_returns():
    ...
    hfi = pd.read_csv(r'C:\Users\analyst2.LCA\OneDrive - crescendoam.com\Desktop\Feb 2023\data\edhec-hedgefundindices.csv', header=0, index_col=0, parse_dates=True)
...
def get_ind_returns():
    ind = pd.read_csv(r'C:\Users\analyst2.LCA\OneDrive - crescendoam.com\Desktop\Feb 2023\data\ind30_m_vw_rets.csv',header=0, index_col=0, parse_dates=True)/100
...
def get_ind_nfirms():
    ind = pd.read_csv(r'C:\Users\analyst2.LCA\OneDrive - crescendoam.com\Desktop\Feb 2023\data\ind30_m_nfirms.csv',header=0, index_col=0, parse_dates=True)
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind

def get_ind_size():
    ind = pd.read_csv(r'C:\Users\analyst2.LCA\OneDrive - crescendoam.com\Desktop\Feb 2023\data\ind30_m_size.csv',header=0, index_col=0, parse_dates=True)
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind
...
def annulized_return(r):
    n_months = r.shape[0]
    return_per_day = (r/100+1).prod()**(1/n_months) - 1
    annualized_return = (r/100+1).prod()**(12/n_months) - 1
...
def plot_ef(n_points, er, cov, show_cml = False, style=".-", riskfree_rate = 0, show_ew=False, show_gmv=False):
    ...
    ax = ef.plot.line(x="Volatility", y ="Returns", style=style,color = "#A2142F")
    if show_ew:
        n = er.shape[0]
        ...
        #display EW
        ax.plot([vol_ew], [r_ew], color = "#A2142F", marker="o", markersize=10)
...
def summary_stats(r, riskfree_rate=0.03, periods_per_year=12):
    """
    Return a DataFrame that contains aggregated summery stats for the returns in the columns of r
    """
    ann_r = r.aggregate(annualized_rets, periods_per_year=periods_per_year)
    ann_vol = r.aggregate(annualized_vol, periods_per_year=periods_per_year)
    ann_sr = r.aggregate(sharp_ratio, riskfree_rate=riskfree_rate, periods_per_year=periods_per_year)
...
def bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
    ...
    and the bond value is couputed over time.
    i.e. The index 
    """
...
def bond_total_return(monthly_prices, principal, coupon_rate, coupons_per_year):
    """
    Computed the total return of a Bond based on monthly bond prices and coupon payments
...
def floor_allocator(psp_r, ghp_r, floor, zc_prices, m=3):
    ...
    Uses a CPPI-style dynamic risk budgeting algorithm by inevsting a multiple
    of the cushion in the PSP
    Returns a DataFrame with the same shape as the psp/ghp repersenting the weights in the PSP
    """
    ...
    for step in range(n_steps):
        floor_value = floor * zc_prices.iloc[step] ## PV of Floor assuming todat's rates and flat YC
        cushion = (account_value - floor_value)/account_value
